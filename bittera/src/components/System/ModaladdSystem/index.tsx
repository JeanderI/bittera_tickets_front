...
import { ContainerButton } from "../../Section/styles.ts";
...
export const ModalAddSystem = ({
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        if (response.status === 409) {
          toast.error(
            "Erro ao registrar sistema: Conflito. sistema já existe."
          );
  ...
  return (
    <Modal toggleModal={toggleSystemModal}>
      <FormContainer onSubmit={handleSubmit(createSystem)}>
        <div>
        ...
        <ContainerButton>
          <Button type="submit">Novo sistema</Button>
        </ContainerButton>
      </FormContainer>
    </Modal>
  );
